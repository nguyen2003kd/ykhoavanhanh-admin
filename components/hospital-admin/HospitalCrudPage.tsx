                  ...
                  <table className="min-w-max table-auto border-separate border-spacing-0 text-sm">
                    <thead>
                      <tr className="bg-slate-50/90">
                        <th className="sticky left-0 z-10 border-b border-gray-200 bg-slate-50/90 px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-500 whitespace-nowrap">
                          STT
                        </th>
                        {columns.map((column) => (
                          <th
                            key={column.title}
                            className={`border-b border-gray-200 px-4 py-3 text-left text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-500 whitespace-nowrap ${column.headerClassName ?? column.className ?? ""}`}
                          >
                            {column.title}
                          </th>
                        ))}
                        {showActions && (
                          <th className="sticky right-0 z-10 border-b border-gray-200 bg-slate-50/90 px-4 py-3 text-right text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-500 whitespace-nowrap">
                            Thao tác
                          </th>
                        )}
                      </tr>
                    ...
                    <tbody>
                      {pagedItems.map((item, index) => (
                        ...
                        >
                          <td className="sticky left-0 z-[1] border-b border-gray-100 bg-inherit px-4 py-4 align-top whitespace-nowrap">
                          ...
                          {columns.map((column) => (
                            <td
                              key={column.title}
                              className={`min-w-[120px] border-b border-gray-100 px-4 py-4 align-top text-[15px] leading-7 text-slate-700 ${column.cellClassName ?? column.className ?? ""}`}
                            >
                          ...
                          {showActions && (
                            <td className="sticky right-0 z-[1] border-b border-gray-100 bg-inherit px-4 py-4">
