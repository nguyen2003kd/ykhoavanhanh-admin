# Rooms API — Phòng khám + gán dịch vụ (his_room_services)
...
`exam_area` (tên khu khám) và `his_room_services[].service` (dịch vụ của phòng).
...
### Response (data)
Row `his_rooms` vừa tạo — **không kèm** `exam_area`/`his_room_services` (chỉ
`create()` trần, không include; phòng mới tạo cũng chưa gán dịch vụ nào).
```json
{
  "status": "success",
...
mềm), kèm `exam_area` + `his_room_services[].service` giống `GET /rooms`.
...
### Response (data)
Row đã update — **không kèm** `exam_area`/`his_room_services` (dùng
`BaseProvider.put()` trần, không include).
...
4. **`POST`/`PUT /rooms/{id}` không trả kèm `exam_area`/`his_room_services`**
