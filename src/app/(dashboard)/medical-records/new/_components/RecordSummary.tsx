import { CalendarDays, ClipboardList, IdCard, Phone, ShieldCheck, UserRound } from "lucide-react";
...
export function RecordSummary({ ctrl }: { ctrl: NewMedicalRecordController }) {
  const { router, selectedPatient, form, isSaving } = ctrl;
      ...
      <Card className="sticky top-6 rounded-2xl border-slate-200/80 shadow-[0_1px_3px_0_rgba(0,0,0,0.04)]">
        ...
        <CardContent className="space-y-5">
          ...
          <div className="grid grid-cols-[1fr_2fr] gap-3 pt-1">
            ...
            <Button type="submit" variant="primary" className="h-11 rounded-xl" disabled={isSaving || !selectedPatient}>
              {isSaving ? <><Spinner size="sm" className="mr-2" />Đang lưu...</> : "Tạo hồ sơ"}
            </Button>
          </div>
        </CardContent>
      </Card>
